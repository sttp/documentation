...
Filter expressions operate against in-memory [data set](https://github.com/sttp/documentation/blob/master/docs/data-tables), not a backend database. The filtering syntax used in conjunction with a data set is designed for read-only operations and exposes no update functionality. Because of this, filter operations are not subject to SQL injection attacks or other security concerns typically associated with SQL implementations.
